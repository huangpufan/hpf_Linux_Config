...
| 在非 `hpf` 账户静默执行个人 bootstrap | 会在非预期机器上生成/上传 SSH key，并切换 GitHub git protocol | agent 必须先问用户是否允许；获准后设置 `HPF_BOOTSTRAP_CONFIRM_PERSONAL=yes` 和 `HPF_GIT_EMAIL`；`hpf` 账户直接执行 | playbook、`bootstrap.sh` |
